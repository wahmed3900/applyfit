"""
ApplyFit — FastAPI backend (Render).
Routes and JSON shapes match the frontend (index.html on Vercel) exactly.
...
never hardcode them here):
    ...
    MONGODB_URI   (optional — subscription checks default to "allowed" without it)
"""
...
if not ANTHROPIC_API_KEY:
    log.warning("ANTHROPIC_API_KEY not set — Claude calls will fail at request time")
...
# CLAUDE MODEL
# ============================================================
# Use GET /debug/models to list every model ID your Anthropic key can access.
CLAUDE_MODEL = "claude-sonnet-4-6"

# ============================================================
# APP + CORS
# ============================================================
app = FastAPI(title="ApplyFit", version="0.2.0")
...
# HELPERS
# ============================================================
def require_claude() -> anthropic.Anthropic:
    if client is None:
...
def claude_json(system: str, user: str, max_tokens: int = 2000) -> dict:
    ...
    resp = require_claude().messages.create(
        model=CLAUDE_MODEL,
...
def debug_models():
    ...
    try:
        models = require_claude().models.list()
